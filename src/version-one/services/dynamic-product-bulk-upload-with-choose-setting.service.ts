...
export const addDynamicChooseSettingProductsFromCSVFile = async (req: Request) => {
  try {
        ...
        .then((rows: any[]) => {
          ...
          rows.forEach((row: any) => {
            let data = {
              ...
              metal_tone: row[25],
              metal_weight: row[26],
              band_metal_weight: row[27],
              quantity: row[28],
              diamond_is_band: row[29],
              stone: row[30],
              shape: row[31],
              mm_size: row[32],
              color: row[33],
              ...
              meta_tag: row[43],
              additional_detail: row[44],
              certification: row[45],
              shipping_days: row[46],
            };
...
const getProductsFromRows = async (rows: any,client_id:number, req: Request) => {
 ...
 const {Tag,SettingTypeData,SizeData,LengthData,Collection,CategoryData,MetalMaster, GoldKarat, MetalTone, StoneData, DiamondGroupMaster, DiamondShape, MMSizeData, Colors, ClarityData, CutsData,DiamondCaratSize, Product} = initModels(req);
        ...
        productList.push({
          ...
          is_single:
            row.is_single && row.is_single.toString().toLowerCase() === "true"
              ? "1"
              : "0",
          is_3d_product: row.is_3d_product && row.is_3d_product == "true" ? true : false,
          is_choose_setting: 1,
          is_band:
...
const setMetalOptions = async (productList: any, metalList: any, karatList: any, metalToneList: any) => {
  ...
  for (i = 0; i < length; i++) {
    ...
    for (k = 0; k < pmoLength; k++) {
      ...
      if (strMetalTone.code !== DEFAULT_STATUS_CODE_SUCCESS) {
        errors.push({
          product_name: productList[i].name,
          product_sku: productList[i].name,
...
const addProductToDB = async (productList: any, idAppUser: number,client_id:number, req: Request) => {
  ...
  let resProduct,
    ...
    pdoPayload: any = [],
    imgPayload: any = [];
  const {Product, ProductCategory, ProductMetalOption, ProductDiamondOption, ProductImage} = initModels(req);
  try {
    const activityLogs = []
