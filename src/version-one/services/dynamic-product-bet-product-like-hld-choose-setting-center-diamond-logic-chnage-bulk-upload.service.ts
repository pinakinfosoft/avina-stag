...
/*************  ✨ Windsurf Command 🌟  *************/
/**
 * This function adds a new product bulk upload file
 * @param {Request} req - Express request object
 * @returns {Promise<TResponseReturn>} - Promise with a response object
 */
export const addBETDynamicChooseSettingProductsFromCSVFile = async (req: Request) => {
  try {
    ...
    // Create a new product bulk upload file
    ...
    // Process the product bulk upload file
        ...
        .then((rows: any[]) => {
          ...
          rows.shift();
            ...
            let data = {
              ...
              is_single: row[16],
              is_3d_product: row[17],
              is_band: row[18],
              setting_diamond_shapes: row[19],
              setting_diamond_sizes: row[20],
              center_dia_color: row[21],
              center_dia_clarity: row[22],
              size: row[23],
              length: row[24],
              head_no: row[25],
              shank_no: row[26],
              ...
              cut: row[45],
              stone_type: row[46],
              stone_setting: row[47],
              stone_weight: row[48],
              stone_count: row[49],
              gender: row[50],
              meta_title: row[51],
              meta_description: row[52],
              meta_tag: row[53],
              additional_detail: row[54],
              certification: row[55],
              shipping_days: row[56],
            };
...
const getPipedGenderIdFromFieldValue = async (
  ...
  if (notFound.length > 0) {
    return resNotFound({
      message: prepareMessageFromParams(ERROR_NOT_FOUND, [
        ["field_name", `${error} ${notFound.join(",")}`],
      ]),
    });
  } else {
    return resSuccess({ data: idList.join("|") });
...
const getProductsFromRows = async (rows: any,client_id:number, req: Request) => {
  ...
  try {
    ...
    const {Tag,SettingTypeData,SizeData,LengthData,Collection,CategoryData,MetalMaster, GoldKarat, MetalTone, StoneData, DiamondGroupMaster, DiamondShape, MMSizeData, Colors, ClarityData, CutsData,DiamondCaratSize, Product} = await initModels(req);
    ...
    const diamondGroupMasterList = await DiamondGroupMaster.findAll({
      ...
      if (row.is_parent == "1") {
        ...
        const gender: any =
          row.gender && row.gender != "" && row.gender != "null"
        ...
        const settingDiamondShape:any = row.setting_diamond_shapes && row.setting_diamond_shapes != "" ? await getPipedGenderIdFromFieldValue(
          diamondShapeList,
        ...
        if (row.setting_diamond_shapes && settingDiamondShape == null) {
          errors.push({
            product_name: row.name,
            product_sku: row.sku,
        ...
        const settingDiamondSize:any = row.setting_diamond_sizes && row.setting_diamond_sizes != "" ? await getPipedGenderIdFromFieldValue(
          diamondSize,
          row.setting_diamond_sizes.toString(),
          "value",
          "Setting Diamond Size"
        ) : null;

        if (row.setting_diamond_sizes && settingDiamondSize == null) {
          errors.push({
            product_name: row.name,
            product_sku: row.sku,
            error_message: settingDiamondSize.message,
          });
        }

        const metalTone = row.metal_tone && row.metal_tone != "" ? await getPipedIdFromFieldValue(
          metaToneList,
          row.metal_tone,
          "sort_code",
          "Metal Tone"
        ...
        if (row.metal_tone && metalTone == null) {
          errors.push({
            product_name: row.name,
            product_sku: row.sku,
            error_message: prepareMessageFromParams(ERROR_NOT_FOUND, [
              ["field_name", "Metal Tone"],
            ]),
          });
        }
        ...
        productList.push({
          ...
          is_single:
            row.is_single && row.is_single.toString().toLowerCase() === "true"
              ? "1"
              : "0",
          is_3d_product: row.is_3d_product && row.is_3d_product.toString().toLocaleLowerCase() == "true" ? true : false,
          is_choose_setting: 1,
          is_band:
            row.is_band && row.is_band.toString().toLowerCase() === "true"
              ? "1"
              : "0",
          setting_diamond_shapes: settingDiamondShape && settingDiamondShape != null ? settingDiamondShape.data : null,
          setting_diamond_sizes: settingDiamondSize && settingDiamondSize != null ? settingDiamondSize.data : null,
          metal_tone: metalTone && metalTone != null ? metalTone.data : null,
...
const addProductDetailsToProductList = async (
  ...
  if (productList[currentProductIndex]) {
    ...
    if(row.setting_diamond_shapes && row.setting_diamond_sizes && row.center_dia_color){
        const data = {
        shape: row.setting_diamond_shapes,
        stone: 'diamond',
        color: row.center_dia_color,
        mm_size: null,
        clarity: row.center_dia_clarity,
        cut: null,
        diamond_is_band: false,
        stone_type: 'centre',
        stone_setting: null,
        stone_weight: row.setting_diamond_sizes,
    ...
    if (row.stone && row.stone && row.shape !== "" && row.shape !== "") {
      const data = {
        ...
        clarity: row.clarity,
        cut: row.cut,
        diamond_is_band: row.diamond_is_band,
...
const setMetalOptions = async (productList: any, metalList: any, karatList: any, metalToneList: any) => {
  ...
  for (let index = 0; index < productList.length; index++) {
      ...
      let product_metal_list= []
    if (element[0].product9KTList.length > 0) {
      let productMetalWeight = 0;
      let bandMetalWeight = 0
      for (let i = 0; i < element[0].product9KTList.length; i++) {
        if (element[0].product9KTList[i].product_type && element[0].product9KTList[i].product_type.toLocaleLowerCase() == "band") {
          bandMetalWeight += element[0].product9KTList[i].metal_weight
        } else {
          productMetalWeight += element[0].product9KTList[i].metal_weight;
        }
      }
      const metal: any = await getIdFromName(
        element[0].product9KTList[0].metal,
        metalList,
        "name",
        'Metal'
      )
      ...
      if (metal.code !== DEFAULT_STATUS_CODE_SUCCESS) {
        errors.push({
          ...
          product_sku: productList[index].name,
          error_message: metal.message,
        });
      }
        const karat: any = await getIdFromName(
          element[0].product9KTList[0].karat,
          karatList,
          "name",
          "Karat"
        );
      ...
      if (karat.code !== DEFAULT_STATUS_CODE_SUCCESS) {
        errors.push({
          ...
          product_metal_list.push({
            karat: karat.data,
            metal: metal.data,
            metal_weight: Number(Number(productMetalWeight.toFixed(2))),
            band_metal_weight: bandMetalWeight,
            quantity: productList[index].quantity,
            metal_tone: productList[index].metal_tone,
            retail_price: null,
            compare_price: null,
          });
        }
        ...
        if (element[0].product10KTList.length > 0) {
          ...
          let productMetalWeight = 0;
          let bandMetalWeight = 0
          for (let i = 0; i < element[0].product10KTList.length; i++) {
            if (element[0].product10KTList[i].product_type && element[0].product10KTList[i].product_type.toLocaleLowerCase() == "band") {
              bandMetalWeight += element[0].product10KTList[i].metal_weight
            } else {
              productMetalWeight += element[0].product10KTList[i].metal_weight;
            }
          }
          const metal: any = await getIdFromName(
            element[0].product10KTList[0].metal,
            metalList,
            "name",
            'Metal'
          )
          ...
          if (metal.code !== DEFAULT_STATUS_CODE_SUCCESS) {
            ...
            const karat: any = await getIdFromName(
              element[0].product10KTList[0].karat,
              karatList,
              "name",
              "Karat"
            );
          ...
          if (karat.code !== DEFAULT_STATUS_CODE_SUCCESS) {
            errors.push({
              ...
              product_metal_list.push({
                karat: karat.data,
                metal: metal.data,
                metal_weight: Number(productMetalWeight.toFixed(2)),
                band_metal_weight: bandMetalWeight,
                quantity: productList[index].quantity,
                metal_tone: productList[index].metal_tone,
                retail_price: null,
      ...
      if (element[0].product14KTList.length > 0) {
        let productMetalWeight = 0;
          let bandMetalWeight = 0
          for (let i = 0; i < element[0].product14KTList.length; i++) {
            if (element[0].product14KTList[i].product_type && element[0].product14KTList[i].product_type.toLocaleLowerCase() == "band") {
              bandMetalWeight += element[0].product14KTList[i].metal_weight
            } else {
              productMetalWeight += element[0].product14KTList[i].metal_weight;
            }
          }
          const metal: any = await getIdFromName(
            element[0].product14KTList[0].metal,
            metalList,
            "name",
            'Metal'
          )
          ...
          if (metal.code !== DEFAULT_STATUS_CODE_SUCCESS) {
            ...
            const karat: any = await getIdFromName(
              element[0].product14KTList[0].karat,
              karatList,
              "name",
              "Karat"
            );
      ...
      if (element[0].product18KTList.length > 0) {
        let productMetalWeight = 0;
        let bandMetalWeight = 0
        for (let i = 0; i < element[0].product18KTList.length; i++) {
          if (element[0].product18KTList[i].product_type && element[0].product18KTList[i].product_type.toLocaleLowerCase() == "band") {
            bandMetalWeight += element[0].product18KTList[i].metal_weight
          } else {
            productMetalWeight += element[0].product18KTList[i].metal_weight;
          }
        }
        const metal: any = await getIdFromName(
          element[0].product18KTList[0].metal,
          metalList,
          "name",
          'Metal'
        )
        ...
        if (metal.code !== DEFAULT_STATUS_CODE_SUCCESS) {
          ...
          const karat: any = await getIdFromName(
            element[0].product18KTList[0].karat,
            karatList,
            "name",
            "Karat"
          );
      ...
      if (element[0].product22KTList.length > 0) {
        let productMetalWeight = 0;
        let bandMetalWeight = 0
        for (let i = 0; i < element[0].product22KTList.length; i++) {
          if (element[0].product22KTList[i].product_type && element[0].product22KTList[i].product_type.toLocaleLowerCase() == "band") {
            bandMetalWeight += element[0].product22KTList[i].metal_weight
          } else {
            productMetalWeight += element[0].product22KTList[i].metal_weight;
          }
        }
        const metal: any = await getIdFromName(
          element[0].product22KTList[0].metal,
          metalList,
          "name",
          'Metal'
        )
        ...
        if (metal.code !== DEFAULT_STATUS_CODE_SUCCESS) {
          ...
          const karat: any = await getIdFromName(
            element[0].product22KTList[0].karat,
            karatList,
            "name",
            "Karat"
          );
      ...
      if (element[0].productSilverList.length > 0) {
        let productMetalWeight = 0;
        let bandMetalWeight = 0
        for (let i = 0; i < element[0].productSilverList.length; i++) {
          if (element[0].productSilverList[i].product_type && element[0].productSilverList[i].product_type.toLocaleLowerCase() == "band") {
            bandMetalWeight += element[0].productSilverList[i].metal_weight
          } else {
            productMetalWeight += element[0].productSilverList[i].metal_weight;
          }
        }
        const metal: any = await getIdFromName(
          element[0].productSilverList[0].metal,
          metalList,
          "name",
          'Metal'
        )
      ...
      if (element[0].productPlatinumList.length > 0) {
        let productMetalWeight = 0;
        let bandMetalWeight = 0
        for (let i = 0; i < element[0].productPlatinumList.length; i++) {
          if (element[0].productPlatinumList[i].product_type && element[0].productPlatinumList[i].product_type.toLocaleLowerCase() == "band") {
            bandMetalWeight += element[0].productPlatinumList[i].metal_weight
          } else {
            productMetalWeight += element[0].productPlatinumList[i].metal_weight;
          }
        }
        const metal: any = await getIdFromName(
          element[0].productPlatinumList[0].metal,
          metalList,
          "name",
          'Metal'
        )
    ...
    productList[index].product_metal_details = product_metal_list
    }
...
const setDiamondOptions = async (productList: any, stoneSettigList: any, gemstoneList: any, diamondShapeList: any, diamondColorList: any, diamondClarityList: any, diamondCutList: any, mmSizeList: any, diamondSizeList: any, diamondGroupMasterList: any) => {
  ...
  for (i = 0; i < length; i++) {
    ...
    for (k = 0; k < pmoLength; k++) {
      ...
      const mmSize: any = productList[i].product_diamond_options[k].mm_size && productList[i].product_diamond_options[k].mm_size != '' ? await getIdFromName(
      ...
      if (mmSize && mmSize != null && mmSize.code !== DEFAULT_STATUS_CODE_SUCCESS) {
        errors.push({
          product_name: productList[i].name,
          product_sku: productList[i].sku,
        ...
        productList[i].product_diamond_options[k].mm_size = mmSize && mmSize.data ? mmSize.data : null;
      }
      ...
      const diamondCut: any = productList[i].product_diamond_options[k].cut && productList[i].product_diamond_options[k].cut != ""  ? await getIdFromName(
      ...
      if (diamondCut && diamondCut.code !== DEFAULT_STATUS_CODE_SUCCESS) {
        errors.push({
          product_name: productList[i].name,
          product_sku: productList[i].sku,
...
const addProductToDB = async (productList: any, idAppUser: number,client_id:number, req: Request) => {
  ...
  let resProduct,
    ...
    pdoPayload: any = [],
    imgPayload: any = [];
  const {Product, ProductCategory, ProductMetalOption, ProductDiamondOption, ProductImage} = await initModels(req);
  try {
    const activityLogs = []
    for (const product of productList) {
      ...
      resProduct = await Product.create(
        {
          ...
          is_quantity_track:
            product.product_metal_details[0].quantity &&
              product.product_metal_details[0].quantity > 0
              ? true
              : false,
      ...
      productDetails = {...productDetails, products: resProduct }
