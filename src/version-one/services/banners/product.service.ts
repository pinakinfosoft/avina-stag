...
export const addUpdateBannerProducts = async (req: Request) => {
    try {
        const { products = [], banner_type = BANNER_TYPE.best_seller} = req.body
        ...
        try {
            ...
            }], findSection?.dataValues?.id, LogsActivityType.Edit, LogsType.templateOneProductSection, req?.body?.session_res?.id_app_user,trn)
            ...
            }], data.dataValues.id, LogsActivityType.Add, LogsType.templateOneProductSection, req.body.session_res.id_app_user,trn)
        }
...
export const getALlBannerProducts = async (req: Request) => {
    try {
