...
export const addVariantProductIntoWishList = async (req: Request) => {
  try {
    ...
    try {
      ...
      if (product_type == AllProductTypes.Product) {
      ...
      } else if (product_type == AllProductTypes.StudConfigurator) {
        const product: any = await dbContext.query(`
            ${studProductQuery} AND SCP.id = ${product_id} AND SCP.company_info_id = ${company_info_id?.data}
          `,
          {
        ...
        if (req.file) {
          ...
          if (moveFileResult.code !== DEFAULT_STATUS_CODE_SUCCESS) {
            console.log("first moveFileResult", moveFileResult);
            return moveFileResult;
          }
        ...
        ProductWishList = await ProductWish.create(
          {
            ...
            id_karat: product[0].karat_id,
            id_metal: product[0].metal_id,
            id_metal_tone:
              id_metal_tone &&
                id_metal_tone != null &&
                id_metal_tone != undefined &&
      ...
      } else if (product_type == AllProductTypes.PendantConfigurator) {
        const product: any = await dbContext.query(`
            ${pendantProductQuery} AND CPP.id = ${product_id} AND CPP.company_info_id = ${company_info_id?.data}
          `,
          {
            type: QueryTypes.SELECT,
            transaction: trn,
