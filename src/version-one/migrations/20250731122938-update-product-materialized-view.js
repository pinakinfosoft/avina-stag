...
AS
 WITH filtered_pmo AS (
         ...
         SELECT pdo_1.id_product,
            sum((CASE WHEN dgm_1.rate IS NOT NULL AND dgm_1.rate != 0 THEN dgm_1.rate ELSE dgm_1.synthetic_rate END) * pdo_1.weight::double precision * pdo_1.count::double precision) AS sum_price
         ...
         SELECT pdo_1.id_product,
            sum((CASE WHEN dgm_1.rate IS NOT NULL AND dgm_1.rate != 0 THEN dgm_1.rate ELSE dgm_1.synthetic_rate END) * pdo_1.weight::double precision * pdo_1.count::double precision) AS diamond_price
           FROM product_diamond_options pdo_1
             LEFT JOIN diamond_group_masters dgm_1 ON dgm_1.id = pdo_1.id_diamond_group
