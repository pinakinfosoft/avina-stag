...
module.exports = {
  async up (queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`DROP MATERIALIZED VIEW IF EXISTS public.product_list_view;`)

    await queryInterface.sequelize.query(`CREATE MATERIALIZED VIEW IF NOT EXISTS public.product_list_view
 ...
 SELECT products.id,
    ...
    jsonb_agg(DISTINCT jsonb_build_object('id', pdo.id, 'weight', pdo.weight, 'count', pdo.count, 'id_shape',
        CASE
            ...
            ELSE pdo.id_shape
        END, 'id_stone', pdo.id_stone, 'id_color', pdo.id_color, 'id_clarity', pdo.id_clarity, 'id_type', pdo.id_type, 'id_cuts', pdo.id_cut)) AS pdo
   FROM products
