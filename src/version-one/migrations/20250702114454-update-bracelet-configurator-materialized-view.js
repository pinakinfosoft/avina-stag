 ...
 SELECT config_bracelet_products.id,
   ...
   FROM config_bracelet_products
     ...
     LEFT JOIN ( SELECT cpdo.config_product_id,
            COALESCE(sum(pdgm.rate * cpdo.dia_count::double precision * CASE WHEN stone.is_diamond = 1 THEN cpdo.dia_wt ELSE 1 END), 0::double precision) AS diamond_rate
...
CREATE INDEX idx_bracelet_cfg_company_info_id
    ON public.bracelet_configurator_price_view USING btree
    (company_info_id)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_dia_total_wt
    ON public.bracelet_configurator_price_view USING btree
    (dia_total_wt)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_diamond_details_gin
    ON public.bracelet_configurator_price_view USING gin
    (diamond_details jsonb_path_ops)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_hook_type
    ON public.bracelet_configurator_price_view USING btree
    (hook_type)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_id_karat
    ON public.bracelet_configurator_price_view USING btree
    (id_karat)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_id_metal
    ON public.bracelet_configurator_price_view USING btree
    (id_metal)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_metal_weight_type
    ON public.bracelet_configurator_price_view USING btree
    (metal_weight_type COLLATE pg_catalog."default")
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_product_dia_type
    ON public.bracelet_configurator_price_view USING btree
    (product_dia_type)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_product_length
    ON public.bracelet_configurator_price_view USING btree
    (product_length)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_product_style
    ON public.bracelet_configurator_price_view USING btree
    (product_style COLLATE pg_catalog."default")
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_setting_type
    ON public.bracelet_configurator_price_view USING btree
    (setting_type)
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_cfg_stone_combination_type
    ON public.bracelet_configurator_price_view USING btree
    (stone_combination_type COLLATE pg_catalog."default")
    TABLESPACE pg_default;
CREATE INDEX idx_bracelet_configurator_price_view
    ON public.bracelet_configurator_price_view USING btree
    (id)
    TABLESPACE pg_default;
CREATE INDEX idx_diamond_details_gin
    ON public.bracelet_configurator_price_view USING gin
    (diamond_details)
    TABLESPACE pg_default;
      `)
