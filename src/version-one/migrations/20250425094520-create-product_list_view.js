...
AS
 WITH filtered_pmo AS (
         ...
         SELECT product_images.id_product,
           ...
           FROM product_images
          WHERE product_images.is_deleted = '0' AND (product_images.image_type = ANY (ARRAY[1, 4]))
        ), sum_price AS (
         SELECT pdo_1.id_product,
          ...
          WHERE pdo_1.is_deleted = '0' AND (pdo_1.id_type = 2 OR 'undefined'::text <> '1'::text)
         ...
         SELECT pdo_1.id_product,
           ...
           FROM product_diamond_options pdo_1
             LEFT JOIN diamond_group_masters dgm_1 ON dgm_1.id = pdo_1.id_diamond_group
          WHERE pdo_1.is_deleted = '0' AND pdo_1.id_type = 2 AND pdo_1.is_band IS FALSE
          GROUP BY pdo_1.id_product
        )
 SELECT products.id,
    products.name,
   ...
   FROM products
     ...
     LEFT JOIN product_categories ON product_categories.id_product = products.id AND product_categories.is_deleted = '0'
     ...
     LEFT JOIN metal_masters metal_master ON metal_master.id = filtered_pmo.id_metal AND metal_master.is_deleted = '0'
     LEFT JOIN product_diamond_options pdo ON pdo.id_product = products.id AND pdo.is_deleted = '0'
     LEFT JOIN diamond_group_masters dgm ON dgm.id = pdo.id_diamond_group AND dgm.is_deleted = '0'
  ...
  WHERE products.is_deleted = '0' AND products.is_active = '1' AND products.parent_id IS NULL
