...
module.exports = {
  async up (queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`DROP MATERIALIZED VIEW IF EXISTS product_list;`);
...
AS
 ...
 SELECT products.id,
	...
	CASE WHEN products.is_3d_product IS TRUE AND is_choose_setting = '1' THEN  (
    SELECT ss.name
    FROM setting_styles ss
    WHERE ss.id = ANY(string_to_array(products.setting_style_type, '|')::int[]) limit 1
  )  END as design_no,
  ...
  ON setting_styles.id = ANY(string_to_array(products.setting_style_type, '|')::int[])
