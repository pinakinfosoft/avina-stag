    ...
    await queryInterface.sequelize.query(`
        ...
        WITH productmetal AS (
                ...
                SELECT cpdo.config_product_id,
                    ...
                    COALESCE(sum(CASE WHEN lower(cpdo.product_type::text) = 'side'::text THEN pdgm.rate * cpdo.dia_count::double precision * (CASE WHEN pdgm.average_carat IS NOT NULL THEN (pdgm.average_carat) ELSE (cpdo.dia_weight) END) ELSE pdgm.rate * cpdo.dia_count::double precision * cpdo.dia_weight END), 0::double precision) AS diamond_rate,
                ...
                SELECT cp_1.id AS config_product_id,
                    ...
                    ceil(
                        CASE
                            WHEN cp_1.center_dia_type = 1 THEN CASE WHEN dgm.average_carat IS NOT NULL THEN (dgm.rate*dgm.average_carat) ELSE (dgm.rate*cz.value::double precision) END
                            ELSE CASE WHEN dgm.average_carat IS NOT NULL THEN (dgm.synthetic_rate*dgm.average_carat) ELSE (dgm.synthetic_rate*cz.value::double precision) END 
